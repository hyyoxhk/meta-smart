...
stoe_list_images() {
    ...
    local LIST=$(find $metalayer/ -type d \( -name '.git' -o -name 'script*' \) -prune -o -type f -wholename '*/images/*.bb' -not -wholename '*/meta-skeleton/*' | grep '.*/images/.*\.bb' | sort)

    if [ "$filter" = "FILTER" ]; then
        local LAYERS_LIST=$(find $metalayer/ -type d \( -name '.git' -o -name 'script*' \) -prune -o -type f -wholename '*/conf/layer.conf' -not -wholename '*/meta-skeleton/*' | grep '.*/conf/layer\.conf' | sed 's#/conf/layer.conf##' | sort)
...
{
    #extract bsp path
    local machine_path=$(find ${ROOTOE}/$_META_LAYER_ROOT/ -type d \( -name '.git' -o -name '.repo' -o -name 'build*' -o -name 'script*' \) -prune -o -type f -name "$MACHINE.conf" | grep "/machine/$MACHINE.conf" | sed 's|\(.*\)/conf/machine/\(.*\)|\1|')
    if [ -z "$machine_path" ]; then
        echo "$machine_path"
        echo "[ERROR] No '$MACHINE.conf' file available in $_META_LAYER_ROOT"
        echo ""
        return 1
    fi
    ...
    if [ "$(echo $machine_path | wc -w)" -gt 1 ]; then
        echo ""
        echo "[ERROR] Found multiple '$MACHINE.conf' file in $_META_LAYER_ROOT"
        echo ""
        return 1
    fi
...
_choice_formated_configs() {
    local choices=$(find ${ROOTOE}/$_META_LAYER_ROOT/ -type d \( -name '.git' -o -name '.repo' -o -name 'build*' -o -name 'script*' \) -prune -o -type f -wholename "*/conf/$1/*.conf" 2>/dev/null | grep ".*/conf/$1/.*\.conf" | sort | uniq)
...
verify_env() {
    ...
    if [[ ! -d $oe_tmp_pwd/sources/openembedded-core ]] && [[ ! -d $oe_tmp_pwd/sources/poky ]]; then
        ...
        do
            ...
            if [[ -d $oe_tmp_root/sources/openembedded-core ]] || [[ -d $oe_tmp_root/sources/poky ]]; then
        ...
        [ -d $oe_tmp_pwd/sources/poky ] && _BUILDSYSTEM=sources/poky
        [ -d $oe_tmp_pwd/sources/openembedded-core ] && _BUILDSYSTEM=sources/openembedded-core
    fi
    if [[ "$__resultvar" ]]; then
